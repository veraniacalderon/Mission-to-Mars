...
#convert our DataFrame back into HTML
df.to_html()
...
# only want the automated browser to remain active while we're scraping data, turn it off
# browser.quit()
...
for x in range(4):
    ...
    link = hemi_soup.find(lambda tag:tag.name=="a" and "Sample" in tag.text)
    hemi_url = link.get('href')
